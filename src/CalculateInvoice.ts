...
export default class CalculateInvoice {
  ...
  async calculate(cardNumber: string, invoiceCurrency: string, today: Date) {
    ...
    const purchases = await this.invoiceRepository.getPurchases(
      cardNumber,
      month,
      year
    );
    const currencies: any[] = [
      ...new Set(
        purchases.map((purchase: { currency: string }) => purchase.currency)
      ),
    ].filter((currency) => currency !== invoiceCurrency);
    const exchange: { [currency: string]: number } = {};
    for (const currency of currencies) {
      const rate = await this.exchangeGateway.getExchangeRate(
        currency,
        invoiceCurrency
      );
    ...
    return invoice.calculate(invoiceCurrency, exchange);
  }
}
